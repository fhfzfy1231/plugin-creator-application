...
- 审核通过或驳回后通过 Halo 通知系统向申请人的注册邮箱发送结果
- 驳回通知会附带管理员填写的审核留言
- 可通过签名 Webhook 将新申请实时推送到 AstrBot 群聊

旧版本中已保存的文章链接字段会继续保留以兼容历史数据，新申请不再采集或发送文章链接。
...
审核结果邮件复用 Halo 自带的邮件通知器，无需在插件中重复填写 SMTP 信息。请先在 Halo“设置 → 通知设置”中正确配置邮件服务器；申请人的账号需要绑定可用邮箱。用户若未单独修改通知偏好，Halo 会默认使用邮件通知器发送这两类审核结果。
